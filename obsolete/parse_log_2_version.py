...
def parse(src_dir, dst_dir, suffix, isunmon):
    try:
        ...
        pattern1 = '\d{4}/\d{2}/\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[DEBUG\]:\s+\[[a-zA-Z]+\]\[\d+.\d+.\d+.\d+\:\d+\]\s*(?P<tmp>Payload|Dummy)\s*,\s*(?P<real>-?\d+)\s*\+\s*(?P<dummy>-?\d+)\s+bytes at (?P<timestamp>\d{2}:\d{2}:\d{2}.\d{3})\s*'
        ...
        for line in lines:
            ...
            if res:
                timestr, real_bytes, dummy_bytes = res.groups()[3], int(res.groups()[1]), int(res.groups()[2])
                ...
                timestamp = (timestamp - first_time).total_seconds()
                raw_trace.append([timestamp, real_bytes, dummy_bytes])
        # the timestamp could be unordered in rare cases
        raw_trace = sorted(raw_trace, key=lambda x: x[0])
        with open(savefiledir, 'w') as f:
